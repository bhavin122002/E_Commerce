const Hoopearrings = require("../models/hoopearrings");
...
const getAllProducts = async (req, res) => {
  ...
  let apiData = Hoopearrings.find(queryObject);
...
const getAllProductsHoopearrings = async (req, res) => {
  const myData = await Hoopearrings.find(req.query);
  console.log("first.. =>", req.query);
  res.status(200).json({ myData, nbHits: myData.length });
};
