const Braceletsbangles = require("../models/braceletsbangles");
...
const getAllProducts = async (req, res) => {
  ...
  let apiData = Braceletsbangles.find(queryObject);
...
const getAllProductsBraceletsbangles = async (req, res) => {
  const myData = await Braceletsbangles.find(req.query);
  console.log("first.. =>", req.query);
  res.status(200).json({ myData, nbHits: myData.length });
};
